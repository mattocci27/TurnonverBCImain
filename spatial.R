...
#     WSG=trait$WSG)
#
#   ab_t_data <- full_join(ab_data, trait_temp, by = "sp") %>%
#     na.omit%>%
...
#   #             + offset(log(ab1982)),
#   #             family = nbinom,
#   #             data = ab_t_data[ab_t_data$ab1982>1,])
#
#   res_all <- glm.nb(ab2010 ~WSG
#               + moist
#               + convex
#               + slope
#               + WSG:moist
...
#
#   summary(res_all)
#   r2_func <- function(res, n){
#     data <- ab_t_data[ab_t_data$ab1982 > n, ]
#     y <- data$ab2010/data$ab1982
#     residuals <- y - res$fitted.values/data$ab1982
#     # residuals = y - exp(res$linear.predictors)/data$census_1982
#     1 - sum(residuals^2) / sum((y - mean(y))^2)
#   }
#
#   moge[i] <- r2_func(res_all, n = 1)
# }
#
#
#
