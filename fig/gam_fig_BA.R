...
Moist20.rm <-Moist20
slope20.rm <- slope20
convex20.rm <- convex20
###use convex mean
for (i in 1:7){
  WSG100.rm[[i]] <- com.mean.ab(D100ba[[i]][,!(colnames(D100ba[[1]]) %in% c("PIPECO","POULAR"))],trait,"WSG")
  Moist100.rm[[i]] <- com.mean.ab(D100ba[[i]][,!(colnames(D100ba[[1]]) %in% c("PIPECO","POULAR"))],trait,"Moist")
  slope100.rm[[i]] <- com.mean.ab(D100ba[[i]][,!(colnames(D100ba[[1]]) %in% c("PIPECO","POULAR"))],trait,"sp.slope.mean")
  # convex100.rm[[i]] <- com.mean.ab(D100ba[[i]][,!(colnames(D100ba[[1]]) %in% c("ALSEBL","SWARS1"))],trait,"sp.convex.mean")
  convex100.rm[[i]] <- com.mean.ab(D100ba[[i]][,!(colnames(D100ba[[1]]) %in% c("PIPECO","POULAR"))],trait,"sp.convex.mean")
}
...
#last version
pdf("~/Dropbox/MS/TurnoverBCI/fig/fig2_ba.pdf", width = 4.77, height = 4.77)
