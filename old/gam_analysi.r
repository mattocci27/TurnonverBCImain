...
### WSG R2
SS <- sum((moge100r[,"WSG"] - mean(moge100r[,"WSG"]))^2)
PREDS <- sum((r1.r100$gam$fitted - moge100r$WSG)^2)
1-PREDS/SS
##
...
# res.cv <- numeric(k)
  res.cv <- NULL
  i <- 1
  r2.1 <- gamm(WSG ~  s(Time,k=4), random = list(site=~1), data=moge.cross[(35*i+1):350,], correlation = corCAR1())
  ...
  SS <- sum((moge.cross[1:35,"WSG"] - mean(moge.cross[1:35,"WSG"]))^2)
  PREDS <- sum((res3$fitted - res3$WSG)^2)
  res.cv[i] <- 1 - PREDS/SS
...
for (i in 2:(k-1)){
  ...
  r2.1 <- gamm(WSG ~  s(Time,k=4), random = list(site=~1), data=temp.data, correlation = corCAR1())
  res <- data.frame(fitted = r2.1$gam$fitted.values, Time= r2.1$gam$model$Time)
  res2 <- unique(res[order(res$Time),])
  ...
  SS <- sum((moge.cross[K1:K2,"WSG"] - mean(moge.cross[K1:K2,"WSG"]))^2)
  PREDS <- sum((res3$fitted - res3$WSG)^2)
  res.cv[i] <- 1 - PREDS/SS
}

i <- 10
  r2.1 <- gamm(WSG ~  s(Time,k=4), random = list(site=~1), data=moge.cross[1:35*k,], correlation = corCAR1())
  res <- data.frame(fitted = r2.1$gam$fitted.values, Time= r2.1$gam$model$Time)
  res2 <- unique(res[order(res$Time),])
  res3 <- merge(moge.cross[316:350,],res2,by="Time")
  SS <- sum((moge.cross[316:350,"WSG"] - mean(moge.cross[316:350,"WSG"]))^2)
  PREDS <- sum((res3$fitted - res3$WSG)^2)
  res.cv[i] <- 1 - PREDS/SS
...
list(res.cv=res.cv,res.data=data.frame(CV,R2))




save.image("~/Dropbox/MS/TurnoverBCI/gam_analysis.RData")
